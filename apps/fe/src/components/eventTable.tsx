...
function deleteEvent(eventsApi:EventsApi, eventId: number){
 return eventsApi
  ...
  const EventTable: React.FC = () => {
        ...
        <Table.Tr key={event.id}>
          ...
          <Table.Td>
            <Flex gap="md">
              ...
              <ActionIcon styles={{root:{backgroundColor:"red"}}} onClick={ () => deleteEventMutation.mutate(event.id)}>
                <IconTrash />
              </ActionIcon>
            </Flex>
          </Table.Td>
    ...
    return (
        <Container>
        <LoadingOverlay visible={getEvents.isLoading} zIndex={1000} overlayProps={{ radius: "sm", blur: 2 }} />
            <Flex justify={'end'}>
            <Button
                my="md"
